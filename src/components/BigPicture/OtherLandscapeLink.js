...
const CardLink = ({ url, children }) => {
  ...
  const to =
    url.indexOf("http") === 0 ? url : stringifyParams({ mainContentMode: url });

  return (
    <Component to={to} style={{ display: "flex", flexDirection: "column" }}>
      {children}
    </Component>
  );
};

const OtherLandscapeLink = function ({
  top,
  left,
  height,
  width,
  ...
  if (layout === "subcategory") {
    return (
      <div style={{ width, left, height, top, position: "absolute" }}>
        <CardLink url={url}>
          <div
            style={{
              position: "absolute",
