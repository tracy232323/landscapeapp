...
const HomePage = (_) => {
  ...
  const titlePrefix = isBigPicture
    ? landscapeSettings.isMain
      ? ""
      : landscapeSettings.name
    : "Card Mode";
  const title = [titlePrefix, settings.global.meta.title]
    .filter((_) => _)
    .join(" - ");
    ...
    <>
      ...
      <div
        ...
        className={classNames("app", {
          "filters-opened": sidebarVisible,
          "big-picture": isBigPicture,
        })}
      >
        <div
          style={{
            marginTop: isIphone && selectedItemId ? -lastScrollPosition : 0,
          }}
          className={classNames(
            { "iphone-scroller": isIphone && selectedItemId },
            "main-parent"
          )}
        >
          ...
          <div className={classNames("main", { embed: isEmbed })}>
            <div className="disclaimer">
              <span
                dangerouslySetInnerHTML={{ __html: settings.home.header }}
              />
              Please{" "}
              <OutboundLink to={`https://github.com/${settings.global.repo}`}>
                open
              </OutboundLink>{" "}
              a pull request to correct any issues. Greyed logos are not open
              source. Last Updated: {process.env.lastUpdated}
            </div>
            <Summary />
