...
const getLastCommitSha = function () {
  return require("child_process")
    .execSync(`cd '${projectPath}' && git log -n 1 --format=format:%h`)
    .toString("utf-8")
    .trim();
};
...
async function main() {
  ...
  const sizes = landscapeSettingsList.reduce((acc, landscapeSettings) => {
    const { fullscreenWidth, fullscreenHeight } =
      calculateSize(landscapeSettings);
  ...
  await Promise.mapSeries([previews, full_sizes], async function (series) {
    for (const pageInfo of series) {
      const { url, deviceScaleFactor, fileName, pdfFileName, basePath } =
        pageInfo;
      ...
      await page.screenshot({
        path: resolve(...imagesPath, fileName),
        fullPage: false,
      });
      if (pdfFileName) {
