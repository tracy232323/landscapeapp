...
import {
  extractSavedImageEntries,
  fetchImageEntries,
  removeNonReferencedImages,
} from "./fetchImages";
import {
  extractSavedCrunchbaseEntries,
  fetchCrunchbaseEntries,
} from "./crunchbase";
...
if (key.toLowerCase() === "easy") {
  ...
  try {
    require("fs").unlinkSync(
      path.resolve(projectPath, "processed_landscape.yml")
    );
  } catch (_ex) {
  ...
  console.info(
    "Unknown level. Should be one of easy, medium, hard or complete"
  );
}
...
const aggregateContributions = (repos) => {
  const contributions = repos.map((repo) =>
    repo.contributions.split(";").map((count) => parseInt(count))
  );
...
const getRepoWithLatestCommit = (repos) => {
  return repos.sort(
    (a, b) => new Date(b.latest_commit_link) - new Date(a.latest_commit_link)
  )[0];
};
...
async function main() {
  ...
  if (settings.global.skip_crunchbase) {
    ...
    // 从 processed_landscape.yml 中提取 crunchbase 信息
    savedCrunchbaseEntries = await extractSavedCrunchbaseEntries();
  ...
  if (!process.env.TWITTER_KEYS) {
    console.info(
      "TWITTER_KEYS not provided. We will not be able to fetch latest tweet dates"
    );
  }
  ...
  const fetchBestPracticeEntries = useBestPracticesCache
    ? fetchBestPracticeEntriesWithIndividualUrls
    : fetchBestPracticeEntriesWithFullScan;
  ...
  const newProcessedLandscape = tree.map(function (node) {
    if (node && node.item === null) {
      ...
      if (node.organization) {
        ...
        if (node.crunchbase) {
          addFatal(
            `the project does not use a crunchbase, but a crunchbase ${node.crunchbase} is present for a ${node.name}`
          );
        }
      } else if (node.unnamed_organization) {
        node.crunchbase = settings.global.self;
        node.crunchbase_data = _.clone({
        ...
        if (settings.global.skip_crunchbase) {
          addFatal(
            `organization field is not provided for a ${node.name}. Crunchbase fetching is disabled for this project`
          );
        } else {
          var crunchbaseInfo = _.clone(
            _.find(crunchbaseEntries, { url: node.crunchbase })
          );
          if (crunchbaseInfo) {
      ...
      //github
      var githubEntry = _.clone(
        _.find(githubEntries, { url: node.project_org }) ||
      ...
      if (githubEntry) {
        ...
        const additionalRepos = [
          ...(node.additional_repos || []),
          ...(githubEntry.repos || []),
        ]
          .map(
            (node) =>
              node && _.find(githubEntries, { url: node.repo_url || node.url })
          )
        ...
        if (
          (node.project_org || node.additional_repos) &&
          repos.length > 0 &&
          !githubEntry.cached
        ) {
          ...
          node.github_data.stars = repos.reduce(
            (acc, { stars }) => acc + stars,
            0
          );
          node.github_data.languages = aggregateLanguages(repos);

          const { latest_commit_link, latest_commit_date } =
            getRepoWithLatestCommit(repos);
        ...
        node.repos = githubEntry.cached
          ? githubEntry.repos
          : repos.map((repo) => ({ url: repo.url, stars: repo.stars }));
        ...
        node.repos = [
          node.repo_url,
          ...(node.additional_repos || []).map(({ repo_url }) => repo_url),
        ].map((url) => ({ url }));
      }

      //github start dates
      var dateEntry = _.clone(
        _.find(startDateEntries, { url: node.project_org || node.repo_url })
      );
      if (dateEntry) {
        node.github_start_commit_data = dateEntry;
        const mainRepo = _.clone(
          _.find(startDateEntries, { url: node.repo_url })
        );
        const repos = [
          mainRepo,
          ...(node.additional_repos || []),
          ...
          .filter((_) => _)
          .map((node) =>
            _.find(startDateEntries, { url: node.repo_url || node.url })
          )
          .filter((x) => !!x);
      ...
      // best practicies
      let bestPracticeEntry = _.clone(
        _.find(bestPracticeEntries, function (x) {
          if (!x) {
            return false;
          }
      ...
      if (
        !bestPracticeEntry ||
        _.isEmpty(bestPracticeEntry) ||
        (_.isUndefined(bestPracticeEntry.badge) &&
          _.isUndefined(bestPracticeEntry.percentage))
      ) {
        bestPracticeEntry = {
      ...
      const twitterEntry = _.clone(
        _.find(twitterEntries, {
          url: twitter,
        })
      );
      if (twitterEntry) {
