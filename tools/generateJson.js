...
const processedLandscape = load(
  readFileSync(`${projectPath}/processed_landscape.yml`)
);
const { crunchbase_overrides = {} } = load(
  readFileSync(`${projectPath}/landscape.yml`)
);
...
async function getMembers() {
  ...
  if (hasMembershipFile) {
    await failOnSingleError(
      `members.yml is not supported anymore. Instead, we need a 'membership' key in a 'global' setting of a settings.yml file`
    );
  }
  if (!membershipCategoryName) {
    await failOnSingleError(
      `membership category (global.membership in settings.yml) is not present. Please add a category to the settings.yml file`
    );
  }
  ...
  tree.map(function (node) {
    if (
      node &&
      node.category === null &&
      node.name === settings.global.membership
    ) {
      node.subcategories.forEach(function (subcategory) {
        directResult[subcategory.name] = subcategory.items.map((item) =>
          getItemMembershipKey(item)
        );
      });
    }
  });
...
async function main() {
  ...
  tree.map(function (node) {
    if (node && node.item === null) {
      ...
      const getLicense = function () {
        if (
          (node.hasOwnProperty("open_source") && !node.open_source) ||
          (!node.github_data && !node.other_repo_url)
        ) {
      ...
      const getAmount = function () {
        ...
        if (
          node.crunchbase_data &&
          node.crunchbase_data.funding &&
          node.crunchbase_data.funding > 0
        ) {
      ...
      items.push({
        ...
        firstCommitDate: formatDate(
          (node.github_start_commit_data || {}).start_date
        ),
        firstCommitLink: getCommitLink(
          (node.github_start_commit_data || {}).start_commit_link
        ),
        latestCommitDate: formatDate(
          (node.github_data || {}).latest_commit_date
        ),
        latestCommitLink: getCommitLink(
          (node.github_data || {}).latest_commit_link
        ),
        releaseDate: formatDate((node.github_data || {}).release_date),
        releaseLink: (node.github_data || {}).release_link,
        commitsThisYear: _.sum(
          ((node.github_data || {}).contributions || "")
            .split(";")
            .map((x) => +x)
        ),
        ...
        language:
          (((node.github_data || {}).languages || [])[0] || {}).name || null,
        stars: (node.github_data || {}).stars,
        license: getLicense(),
        headquarters: getHeadquarters(),
        twitter: getTwitter(),
        latestTweetDate: formatDate(
          (node.twitter_data || {}).latest_tweet_date
        ),
        ...
        industries:
          (crunchbase_overrides[node.crunchbase] || {}).industries ||
          (node.crunchbase_data || {}).industries ||
          [],
        enduser: isEndUser(),
      });
    }
  ...
  const itemsWithExtraFields = items.map(function (item) {
    ...
    if (item.crunchbase_data) {
      item.crunchbaseData.numEmployeesMin =
        item.crunchbaseData.num_employees_min;
      item.crunchbaseData.numEmployeesMax =
        item.crunchbaseData.num_employees_max;
  ...
  if (settings.global.flags.companies) {
    ...
    const hasCompanyCategory = (function () {
      ...
      tree.map(function (node) {
        if (
          node &&
          node.category === null &&
          node.name === settings.global.flags.companies
        ) {
          result = true;
        }
      });
    ...
    if (!hasCompanyCategory) {
      await failOnSingleError(
        `can not find a category with name: "${settings.global.flags.companies}". We use that category to get a list of member companies`
      );
    }
  ...
  if (settings.global.flags.hide_license_for_categories) {
    _.each(itemsWithExtraFields, function (item) {
      if (
        settings.global.flags.hide_license_for_categories.indexOf(
          item.category
        ) !== -1
      ) {
  ...
  await Promise.mapSeries(
    _.values(_.groupBy(itemsWithExtraFields, "name")),
    async function (duplicates) {
      if (
        duplicates.length > 1 &&
        duplicates.find(({ allow_duplicate_repo }) => !allow_duplicate_repo)
      ) {
        hasDuplicates = true;
        await Promise.mapSeries(duplicates, async function (duplicate) {
          await failOnMultipleErrors(
            `Duplicate item: ${duplicate.organization} ${duplicate.name} at path ${duplicate.path}`
          );
        });
      }
    }
  ...
  await Promise.mapSeries(
    _.values(
      _.groupBy(
        itemsWithExtraFields.filter((x) => !!x.repo_url),
        "repo_url"
      )
    ),
    async function (duplicates) {
      if (
        ...
        await Promise.mapSeries(duplicates, async function (duplicate) {
          await failOnMultipleErrors(
            `Duplicate repo: ${duplicate.repo_url} on ${duplicate.name} at path ${duplicate.path}`
          );
        });
      }
    }
  ...
  await Promise.mapSeries(itemsWithExtraFields, async function (item) {
    if (!item.crunchbaseData) {
      ...
      await failOnMultipleErrors(
        `${item.name} either has no crunchbase entry or it is invalid`
      );
    }
  });
  ...
  await Promise.mapSeries(itemsWithExtraFields, async function (item) {
    if (
      item.crunchbase &&
      item.crunchbase.indexOf("https://www.crunchbase.com/organization/") !== 0
    ) {
      hasBadCrunchbase = true;
      await failOnMultipleErrors(
        `${item.name}  has a crunchbase ${item.crunchbase} which does not start with 'https://www.crunchbase.com/organization'`
      );
    }
  });
  if (hasBadCrunchbase) {
  ...
  await Promise.mapSeries(itemsWithExtraFields, async function (item) {
    if (!item.homepage_url) {
      hasBadHomepage = true;
      await failOnMultipleErrors(
        `${item.name}  has an empty or missing homepage_url`
      );
    } else if (
      !item.homepage_url.startsWith("http://") &&
      !item.homepage_url.startsWith("https://")
    ) {
      hasBadHomepage = true;
      await failOnMultipleErrors(
        `${item.name}  homepage_url ${item.homepage_url} should start from http:// or https://`
      );
    }
  });
  if (hasBadHomepage) {
  ...
  await Promise.mapSeries(itemsWithExtraFields, async function (item) {
    if (item.twitter && !item.latestTweetDate) {
      if (item.latestTweetDate === null) {
        console.info(
          `Warning: ${item.name} has a twitter ${item.twitter} with no entries`
        );
      } else {
        console.info(
          `Warning: ${item.name} has a twitter ${item.twitter} which is invalid or we just can not fetch its tweets`
        );
      }
    }
  });
  ...
  await Promise.mapSeries(itemsWithExtraFields, async function (item) {
    if (item.twitter && item.twitter.split("/").slice(-1)[0] === "") {
      await failOnMultipleErrors(
        `${item.name} has a twitter ${item.twitter} which ends with /`
      );
      hasWrongTwitterUrls = true;
    }
    if (
      item.twitter &&
      item.twitter.indexOf("https://twitter.com/") !== 0 &&
      item.twitter.indexOf("http://twitter.com/") !== 0
    ) {
      await failOnMultipleErrors(
        `${item.name} has a twitter ${item.twitter} which does not start with https://twitter.com/ or http://twitter.com/`
      );
      hasWrongTwitterUrls = true;
    }
  });
  ...
  await Promise.mapSeries(itemsWithExtraFields, async function (item) {
    if (!item.image_data) {
      ...
      const imageFileName =
        `${projectPath}/cached_logos/` + item.image_data.fileName;
      if (!require("fs").existsSync(imageFileName)) {
        await failOnMultipleErrors(
          `Item ${item.name} does not have a file ${imageFileName} on the disk`
        );
        hasBadImages = true;
      } else {
        ...
        if (fileSize < 100) {
          await failOnMultipleErrors(
            `Item ${item.name} has a file ${imageFileName} size less than 100 bytes`
          );
          hasBadImages = true;
        }
      }
  ...
  async function removeNonReferencedImages() {
    ...
    const allowedFiles = itemsWithExtraFields
      .map((e) => e.logo)
      .filter((e) => !!e);
  ...
  await Promise.mapSeries(
    settings.big_picture.main.elements,
  ...
  await Promise.mapSeries(itemsWithExtraFields, async function (item) {
    if (!_.isEmpty(item.twitter) && item.twitter.indexOf(" ") !== -1) {
      ...
      await failOnMultipleErrors(
        `${item.name} has a twitter ${item.twitter} with spaces`
      );
    }
    if (!_.isEmpty(item.crunchbase) && item.crunchbase.indexOf(" ") !== -1) {
      hasBadFieldWithSpaces = true;
      await failOnMultipleErrors(
        `${item.name} has a crunchbase ${item.crunchbase} with spaces`
      );
    }
    if (!_.isEmpty(item.repo_url) && item.repo_url.indexOf(" ") !== -1) {
      hasBadFieldWithSpaces = true;
      await failOnMultipleErrors(
        `${item.name} has a repo_url ${item.repo_url} with spaces`
      );
    }
    if (
      !_.isEmpty(item.homepage_url) &&
      item.homepage_url.indexOf(" ") !== -1
    ) {
      hasBadFieldWithSpaces = true;
      await failOnMultipleErrors(
        `${item.name} has a homepage_url ${item.homepage_url} with spaces`
      );
    }
  });
  ...
  _.each(itemsWithExtraFields, function (item) {
    const membership = (function () {
      // direct membership
      const directMembership = _.findKey(
        members,
        (v) => v && v.indexOf(getItemMembershipKey(item)) !== -1
      );
      if (directMembership) {
        return directMembership;
      }
      const parentWithMembership = _.find(
        item.crunchbaseData.parents,
        function (parent) {
      ...
      if (parentWithMembership) {
        ...
        const membership = _.findKey(
          members,
          (v) => v && v.indexOf(parentWithMembership) !== -1
        );
        console.info(
          `Assigning ${membership} membership on ${item.name} (${myName}) because its parent ${parentName} has ${membership} membership`
        );
        return membership;
      }
      return false;
  ...
  const hostedCategories = settings.relation.values
    .filter(({ id }) => id === "hosted")
  ...
  itemsWithExtraFields.forEach((item) => {
    ...
    if (
      hostedCategories.includes(project) &&
      crunchbase !== settings.global.self &&
      !parents.includes(settings.global.self)
    ) {
      failOnMultipleErrors(
        `Project ${name} has been added to ${project} category but does not belong to ${settings.global.short_name}`
      );
    }
  });
  ...
  const extractOptions = function (name) {
    return _.chain(itemsWithExtraFields)
      .map(function (x) {
        return x[name];
      })
      .filter(function (x) {
  ...
  const getGroupingLabel = function ({ category, node }) {
    const categoriesToHide =
      (settings.global.flags || {}).hide_category_from_subcategories || [];
  ...
  const generateHeadquarters = function () {
    const values = _.uniq(
      itemsWithExtraFields.map(function (item) {
        return {
          headquarters: item.headquarters,
          country: item.crunchbaseData.country,
        };
      })
    );
    const grouped = _.groupBy(values, (x) => x.country);
    const keys = _.sortBy(_.keys(grouped), (country) =>
      country === "Antarctica" ? "ZZZ" : country
    );
    const result = [];
    _.each(keys, function (key) {
      ...
      const stateAndCity = (x) =>
        x.country === "United States"
          ? x.headquarters.split(", ")[1] + x.headquarters.split(", ")[0]
          : x.headquarters.split(", ")[0];
      _.each(
        _.orderBy(children, (x) => stateAndCity(x)),
        function (record) {
          result.push({
  ...
  const generateLicenses = function () {
    ...
    return [
      ...
      otherLicenses.map(function (license) {
        return {
          ...license,
  ...
  const generateCrunchbaseSlugs = () => {
    const urls = _.flatten(
      itemsWithExtraFields.map(({ crunchbase, crunchbaseData }) => [
        crunchbase,
    ...
    const slugs = urls
      .filter((url) => url)
      .map((crunchbaseUrl) => crunchbaseUrl.split("/").pop());
    return [...new Set(slugs)].sort();
  };

  const generateLanguages = () => {
    const languages = _.flatten(
      itemsWithExtraFields.map(({ github_data }) =>
        ((github_data || {}).languages || []).map((x) =>
  ...
  const generateCompanyTypes = () => {
    const companyTypes = [
      ...new Set(
        itemsWithExtraFields.map(
          ({ crunchbaseData }) => (crunchbaseData || {}).company_type
        )
      ),
    ];
  ...
  const generateIndustries = () => {
    const industries = [
      ...new Set(itemsWithExtraFields.flatMap(({ industries }) => industries)),
    ];

    return industries
      .sort((str, other) =>
        str.toLowerCase().localeCompare(other.toLowerCase())
      )
  ...
  require("fs").writeFileSync(
    ...
    JSON.stringify(itemsWithExtraFields, null, 2)
  );
  require("fs").writeFileSync(
    `${projectPath}/lookup.json`,
